  ...
  <div class="el-time-spinner" :class="{ 'has-seconds': showSeconds }">
    <template v-if="!arrowControl">
      <optiscroll
          ref="hours"
          style="height: 190px"
          class="el-time-spinner__wrapper el-scrollbar"
          :step="32"
          :autoUpdate="false"
          :realTimeRendering="true"
          :size="scrollHeight('hours')"
          @scroll="scrollEvent('hours', $event)">
        <ul :style="{marginTop: -top.hours + 'px'}"
            @mouseenter.native="emitSelectRange('hours')"
            @mousemove.native="adjustCurrentSpinner('hours')"
            class="el-scrollbar__view el-time-spinner__list">
          <li
      ...
      <optiscroll
          ...
          :autoUpdate="false"
          :realTimeRendering="true"
          :size="scrollHeight('minutes')"
          @scroll="scrollEvent('minutes', $event)">
        <ul :style="{marginTop: -top.minutes + 'px'}"
      ...
      <optiscroll
          ...
          :autoUpdate="false"
          :realTimeRendering="true"
          :size="scrollHeight('seconds')"
          @scroll="scrollEvent('seconds', $event)"
          v-show="showSeconds">
        <ul :style="{marginTop: -top.seconds + 'px'}"
...
export default {
  ...
  methods: {
    scrollHeight (type) {
      switch (type) {
        case 'hours': return { sH: 926, cH: 190 }
        case 'minutes': return { sH: 32 * 65, cH: 190 }
        default : return { sH: 32 * 65, cH: 190 }
      }
    },

    scrollEvent (type, e) {
      let value = parseInt(e.scrollTop / 32)
      this.top[type] = value * 32
    ...
    adjustSpinner (type, value) {
      this.$refs[type].scrollTo(0, value * 32)
    },
