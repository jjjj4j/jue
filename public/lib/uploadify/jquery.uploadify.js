...
var swfobject = function () {
  var D = 'undefined', r = 'object', S = 'Shockwave Flash',
      W = 'ShockwaveFlash.ShockwaveFlash', q = 'application/x-shockwave-flash',
      R = 'SWFObjectExprInst', x = 'onreadystatechange', O = window,
      j = document, t = navigator, T = false, U = [h], o = [], N = [], I = [],
      l, Q, E, B, J = false, a = false, n, G, m = true, M = function () {
    ...
    init: function (options, swfUploadOptions) {
      ...
      return this.each(function () {
        ...
        var settings = $.extend({
          ...
          uploader: 'uploadify.php',  // The path to the server-side upload
                                      // script
          ...
          buttonImage: null,               // (String or null) The path to an
                                           ...
                                           // the button
          buttonText: 'SELECT FILES',     // The text to use for the browse
                                          // button
          checkExisting: false,              // The path to a server-side
                                             // script that checks for existing
                                             // files on the server
          debug: false,              // Turn on swfUpload debugging mode
          fileObjName: 'Filedata',         // The name of the file object to
                                           // use in your server-side script
          fileSizeLimit: 0,                  // The maximum size of an
                                             // uploadable file in KB (Accepts
                                             // units B KB MB GB if string, 0
                                             // for no limit)
          fileTypeDesc: 'All Files',        // The description for file types
                                            // in the browse dialog
          fileTypeExts: '*.*',              // Allowed extensions in the browse
                                            // dialog (server-side validation
                                            // should also be used)
          height: 30,                 // The height of the browse button
          itemTemplate: false,              // The template for the file item
                                            // in the queue
          ...
          preventCaching: true,               // Adds a random value to the
                                              ...
                                              // of it (conflicts with existing
                                              // parameters)
          progressData: 'percentage',       // ('percentage' or 'speed') Data
                                            // to show in the queue item during
                                            // a file upload
          ...
          // Events
          overrideEvents: []             // (Array) A list of default event
                                         // handlers to skip
          /*
        ...
        if (flashInstalled) {
          ...
          swfuploadify.queueData = {
            ...
            filesSelected: 0, // The number of files selected in the last
                              // select operation
            filesQueued: 0, // The number of files added to the queue in the
                            // last select operation
            filesReplaced: 0, // The number of files replaced in the last
                              // select operation
            filesCancelled: 0, // The number of files that were cancelled
                               // instead of replaced
            filesErrored: 0, // The number of files that caused error in the
                             // last select operation
            uploadsSuccessful: 0, // The number of files that were successfully
                                  // uploaded
            uploadsErrored: 0, // The number of files that returned errors
                               // during upload
            averageSpeed: 0, // The average speed of the uploads in KB
            queueLength: 0, // The number of files in the queue
            queueSize: 0, // The size in bytes of the entire queue
            uploadSize: 0, // The size in bytes of the upload queue
            queueBytesUploaded: 0, // The size in bytes that have been uploaded
                                   // for the current upload queue
    ...
    cancel: function (fileID, supressEvent) {
      ...
      this.each(function () {
        ...
        if (args[0]) {
          ...
          var item = $('#' + settings.queueID).find('.uploadify-queue-item')
                                              .get(0)
          $item = $(item)
        ...
        if (args.length === 1) {
          ...
          switch (name) {
            ...
            case 'buttonCursor':
              ...
              break
            case 'buttonText':
              $('#' + settings.id + '-button').find('.uploadify-button-text')
                                              .html(value)
              break
            case 'width':
    ...
    onUploadProgress: function (file, fileBytesLoaded, fileTotalBytes) {
      ...
      if ($.inArray('onUploadProgress', settings.overrideEvents) < 0) {
        if (settings.progressData == 'percentage') {
          $('#' + file.id).find('.data')
                          .html(percentage == 100 ? ' - 数据解析中' : ' - ' + percentage + '%')
        } else if (settings.progressData == 'speed' && lapsedTime > 500) {
          $('#' + file.id).find('.data')
                          .html(' - ' + this.queueData.averageSpeed + suffix)
        }
        $('#' + file.id).find('.uploadify-progress-bar')
